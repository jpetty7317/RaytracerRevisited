...
class triangle
{
    ...
    public:
        ...
        const glm::vec3& v0() const { return p0; }
        const glm::vec3& v1() const { return p1; }
        const glm::vec3& v2() const { return p2; }
        const glm::vec3& centroid() const { return cent; }
        ...
        void hit(ray& r)
        {
            ...
            const glm::vec3 normal = (n0 + n1 + n2) / 3.0f;
            if(dot(r.direction(), normal) > 0.0)
                return;

            const glm::vec3 h = glm::cross(r.direction(), e2);
            const float a = dot(h, e1);
            if(a > -0.00001f && a < 0.00001f)
                return;
            
            const float f = 1.0f / a;
            const glm::vec3 s = r.origin() - p0;
            const float u = f * dot(s, h);
            if(u < 0.0f || u > 1.0f)
                return;

            const glm::vec3 q = cross(s, e1);
            ...
            if(t > 0.00001f && t < r.t)
            {
                ...
                r.uv = glm::vec3{x, y, 0.0f};
                r.mat = mat;
            }
        }

        void Project(const std::vector<glm::vec3>& points, const glm::vec3& axis, interval& minMax)
        {
            for(auto& p : points)
            {
        ...
        {
            std::vector<glm::vec3> boxNormals {{1,0,0},
                                            {0,1,0},
                                            {0,0,1}};
            ...
            std::vector<glm::vec3> boxVerts {{box.min()},
                                            {box.max()},
                                            {box.min() + glm::vec3{1,0,0} * box.size().x},
                                            {box.min() + glm::vec3{0,1,0} * box.size().x},
                                            {box.min() + glm::vec3{0,0,1} * box.size().x},
                                            {box.max() - glm::vec3(1,0,0) * box.size().x},
                                            {box.max() - glm::vec3(0,1,0) * box.size().x},
                                            {box.max() - glm::vec3(0,0,1) * box.size().x}};
            ...
            std::vector<glm::vec3> triEdges = {{p0 - p1},
                                            {p1 - p2},
                                            {p2 - p0}};
            ...
            for(int i = 0; i < 3; i++)
            {
                for(int j = 0; j < 3; j++)
                {
                    glm::vec3 axis {glm::cross(triEdges[i], boxNormals[i])};
